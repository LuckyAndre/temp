...
import pretrainedmodels # отсюда можно брать модели
import timm # отсюда можно брать модели
...
if __name__ == '__main__':
    ...
    for model_name in all_models:
        ...
        if not os.path.exists(filename+'.ptl'):
            ...
            model.eval()
            quantized_model = torch.quantization.quantize_dynamic(model, dtype=torch.qint8) # квантование (определенных слоев)
            traced_script_module = torch.jit.trace(model, example)
            #traced_script_module.save(filename+'.pt')
            traced_script_module_optimized = optimize_for_mobile(traced_script_module) # перевод под мобильный телефон (добавилось в torch 1.9)
